...
private:
	...
	//written by Brendan:
	bool delete_leaf(Node* node, Node* parent);
	bool bypass_node(Node* node, Node* parent);
	bool two_child_delete(Node* node);
	void new_node_insert(Node* node, Node* parent, int value);
};
